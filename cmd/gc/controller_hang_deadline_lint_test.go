...
var controllerTestExcludedHangDeadlineLines = map[int]string{
	452:  "input the test feeds a fake server to define the scenario, not a hang detector",
	908:  "negative-assertion window (asserts no watcher poke arrives)",
	958:  "negative-assertion window (asserts no watcher poke arrives, loop body)",
	1487: "bounded best-effort probe with no assertion on either branch",
}
